locals {
  ...
  # Process cloud access role policies
    ...
    for car in var.cloud_access_roles : concat(
      [
        for policy in try(car.aws_iam_policies.user_managed, []) : {
          ...
          type            = "user_managed"
          car_name        = car.name
          key             = "car-${var.ou_name}-${car.name}-${index(car.aws_iam_policies.user_managed, policy)}"
        }
      ],
      [
        for policy in try(car.aws_iam_policies.system_managed, []) : {
          ...
          type            = "system_managed"
          car_name        = car.name
          key             = "car-${var.ou_name}-${car.name}-${policy}"
        }
      ]
    )
  ])
  ...
  scp_map = {
    ...
    scp_template => {
      name         = "${var.ou_name}-SCP-${replace(replace(basename(scp_template), ".tpl", ""), "[^a-zA-Z0-9]", "")}"
      scp_template = scp_template
    }
  }

  # User group name to ID mapping
  user_group_name_to_id = {
    for group_name, group in module.user_groups :
    group_name => group.user_group_id
  }
  ...
  valid_user_or_group = length(var.owner_user_groups) > 0 || length(var.owner_users) > 0
