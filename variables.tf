# Organizational Unit (OU) Metadata

variable "ou_name" {
  description = "Alias of the OU, used as a prefix for resource names."
  type        = string
}
...
variable "labels" {
  description = "A map of labels to assign to the OU."
  type = map(object({
    value = string
    color = string
  }))
...
variable "owner_users" {
  description = "List of owner user IDs for the OU."
  type        = list(number)
}

variable "owner_user_groups" {
  description = "List of owner user group IDs for the OU."
  type        = list(number)
}
